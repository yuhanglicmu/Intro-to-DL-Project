...
def train(rank, args, hp, hp_str):
    ...
    for epoch in range(max(0, last_epoch), args.training_epochs):
        ...
        for i, batch in enumerate(train_loader):
            ...
            if (y.size() != before_y_g_hat.size()):
                if y.size(dim = 0) < before_y_g_hat.size(dim = 0) or y.size(dim = 1) < before_y_g_hat.size(dim = 1) or y.size(dim = 2) < before_y_g_hat.size(dim = 2):
                    before_y_g_hat = before_y_g_hat[0:y.size(dim=0),0:y.size(dim=1),0:y.size(dim=2)]
                else:
                    y = y[0:before_y_g_hat.size(dim=0),0:before_y_g_hat.size(dim=1),0:before_y_g_hat.size(dim=2)]
            ...
            if y_g_hat is not None:
                #change the size
                if (y.size() != y_g_hat.size()):
                    y = y[0:y_g_hat.size(dim=0),0:y_g_hat.size(dim=1),0:y_g_hat.size(dim=2)]
